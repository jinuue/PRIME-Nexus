...
export function renderLogin(container, mode = 'intern') {
  ...
  const sidebarTitle = isHR ? 'HR Administration' : isSupervisor ? 'Supervisor Portal' : 'Intern Portal';
  const sidebarDesc = isHR 
    ? 'Access the HR management system to oversee applications, deployments, and internship programs.'
    : isSupervisor 
      ? 'Manage your assigned interns, monitor their progress, and facilitate their professional growth.'
      : 'Begin your professional journey with PRIME Philippines. Gain industry experience and develop your skills.';
  ...
  document.getElementById('login-form').onsubmit = async (e) => {
    ...
    const roleToLogin = isHR ? 'hr' : (isSupervisor ? 'supervisor' : 'intern');
    const user = loginUser(email, password, roleToLogin);
    if (user) {
      window.APP.login(user);
    } else {
      document.getElementById('auth-error').innerHTML = `<div class="auth-error">${isHR ? 'Access denied. HR credentials required.' : (isSupervisor ? 'Access denied. Supervisor credentials required.' : 'Invalid email or password.')}</div>`;
    }
  };
}
  ...
  container.innerHTML = `
    <div class="auth-page">
      ...
      <div class="auth-sidebar">
        <div class="auth-sidebar-content">
          <h1>Intern Portal</h1>
          <p>Join PRIME Philippines and start your professional journey. Register now to apply for our internship programs.</p>
        </div>
      </div>
    </div>
  `;
